...
void Game::checkClick() {
  if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
    ...
    if (mouseHeld != false)
     ...
     mouseHeld = true;
    if (colides( mousePosView))
      return;

    spawnBall( mousePosView);
...
void Game::spawnBall(sf::Vector2f mPosView) {
  ...
  if (velX == 0)
    rand() % 2 == 0 ? velX += 1 : velX -= 1;
  if (velY == 0)
    rand() % 2 == 0 ? velY += 1 : velY -= 1;

  float offset = rand() % 10 - 5;
  Ball newBall(20.f + offset, mPosView, {velX * 100, velY * 100},
                gravity);
...
void Game::changeColor(int indexA, int indexB) {
  uint8_t r = balls[indexA].rValue, g = balls[indexA].gValue,
  ...
  balls[indexB].rValue += r;
  balls[indexB].gValue += g;
  balls[indexB].bValue += b;

  balls[indexA].ball.setFillColor(sf::Color{
      balls[indexA].rValue, balls[indexA].gValue, balls[indexA].bValue});
  balls[indexB].ball.setFillColor(sf::Color{
      balls[indexB].rValue, balls[indexB].gValue, balls[indexB].bValue});
}
...
void Game::initWindow() {
  sf::VideoMode desktop = sf::VideoMode::getDesktopMode();
   ...
   videoMode.size = {1000, 800};
   window =
      new sf::RenderWindow(sf::VideoMode( videoMode), "Bouncing_balls",
                           sf::Style::Titlebar | sf::Style::Close);

   window->setPosition(
      {(int)((desktop.size.x -  videoMode.size.x) / 2),
       (int)((desktop.size.y -  videoMode.size.y) / 2)});
